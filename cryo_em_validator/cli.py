"""
Command-Line Interface for CryoEM-Validator: Fourier Shell Correlation (FSC) & Local Resolution Agent.
"""
import argparse
import csv
import json
...
from .agents import CryoEMValidatorCoordinator

coordinator = CryoEMValidatorCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="cryo-em-density-validation-agent", description="CryoEM-Validator: Fourier Shell Correlation (FSC) & Local Resolution Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  CRYOEM-VALIDATOR: FOURIER SHELL CORRELATION (FSC) & LOCAL RESOLUTION AGENT")
        print(f"  Domain: Biomedical Scientific Discovery & Multi-Omics | Standard: wwPDB / EMDataBank Cryo-EM Standards")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting CryoEM-Validator: Fourier Shell Correlation (FSC) & Local Resolution Agent on http://{args.host}:{args.port}")
